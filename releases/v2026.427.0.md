...
If you previously ran sandbox execution against the in-tree provider, install `@nessie/plugin-e2b` (or your own provider plugin) to keep using sandboxed runtimes after upgrading. The generic provider plugin contract is documented alongside the new environments subsystem.
