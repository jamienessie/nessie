...
### Manual first publish for `@nessie/ui`
...
npm view @nessie/ui version

# make sure the dist payload is fresh
pnpm --filter @nessie/ui build
...
- If `npm view @nessie/ui version` already returns the same version that is in [`ui/package.json`](../ui/package.json), do not republish. Bump the version or use the normal repo-wide release flow in [`scripts/release.sh`](../scripts/release.sh).
...
- For an organization-scoped package like `@nessie/ui`, the `paperclipai` npm organization must exist and the publisher must be a member with permission to publish to that scope.
...
- every published internal `@nessie/*` dependency referenced by that manifest exists on npm
...
Example for `@nessie/adapter-acpx-local` from the repo root:

```bash
# safe preview
pnpm run release:bootstrap-package -- @nessie/adapter-acpx-local

# one-time first publish from an authenticated maintainer machine
pnpm run release:bootstrap-package -- @nessie/adapter-acpx-local --publish --otp 123456
```
...
1. open `https://www.npmjs.com/package/@nessie/adapter-acpx-local`
