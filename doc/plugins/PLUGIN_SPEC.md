...
- The current runtime ships a small host-provided plugin UI component kit through `@nessie/plugin-sdk/ui`, but does not support plugin asset uploads/reads yet. Treat plugin asset APIs as future-scope ideas, not current implementation promises.
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@nessie/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@nessie/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@nessie/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@nessie/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@nessie/plugin-sdk/ui`
...
The `@nessie/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
### 27.1 `@nessie/plugin-test-harness`
...
```ts
import { createTestHarness } from "@nessie/plugin-test-harness";
...
- `@nessie/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@nessie/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@nessie/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@nessie/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@nessie/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@nessie/plugin-sdk/ui`
