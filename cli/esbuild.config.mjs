 ...
 *
 * Bundles all workspace packages (@nessie/*) into a single file.
...
// to npm and resolved at runtime (e.g. @nessie/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@nessie/")) {
      externals.add(name);
    }
  }
