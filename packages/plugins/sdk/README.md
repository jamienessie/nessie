...
- **Worker SDK:** `@nessie/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@nessie/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@nessie/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@nessie/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@nessie/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@nessie/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@nessie/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, `useHostNavigation`, slot prop types |
| `@nessie/plugin-sdk/ui/hooks` | Hooks only |
| `@nessie/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@nessie/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@nessie/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@nessie/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@nessie/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@nessie/plugin-sdk/types` | Worker context and API types (advanced) |
...
- The host ships a small shared React component kit through `@nessie/plugin-sdk/ui`. Use it for native Paperclip controls; custom React and CSS are still supported.
...
```ts
import { definePlugin, runWorker } from "@nessie/plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@nessie/plugin-sdk`.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@nessie/plugin-sdk`.
...
```tsx
import { usePluginData, usePluginAction } from "@nessie/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@nessie/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@nessie/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@nessie/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@nessie/plugin-sdk/ui";
...
```tsx
import { useHostNavigation } from "@nessie/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@nessie/plugin-sdk/ui";
...
The host provides selected shared UI components through `@nessie/plugin-sdk/ui`.
...
```tsx
import { MarkdownBlock, MarkdownEditor } from "@nessie/plugin-sdk/ui";
...
```tsx
import { FileTree, type FileTreeNode } from "@nessie/plugin-sdk/ui";
...
```tsx
import { AssigneePicker, ProjectPicker } from "@nessie/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@nessie/plugin-sdk/ui`.
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@nessie/plugin-sdk/ui";
...
```ts
import { createTestHarness } from "@nessie/plugin-sdk/testing";
...
```ts
import { createPluginBundlerPresets } from "@nessie/plugin-sdk/bundlers";
...
```ts
import { startPluginDevServer } from "@nessie/plugin-sdk/dev-server";
