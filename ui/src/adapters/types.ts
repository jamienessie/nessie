...
export type { TranscriptEntry, StdoutLineParser, CreateConfigValues } from "@nessie/adapter-utils";

export interface StatefulStdoutParser {
  parseLine: (line: string, ts: string) => import("@nessie/adapter-utils").TranscriptEntry[];
  reset: () => void;
}
...
export interface TranscriptParserSource {
  parseStdoutLine: (line: string, ts: string) => import("@nessie/adapter-utils").TranscriptEntry[];
