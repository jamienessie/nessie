...
test("parseArgs recognizes publish and skip-build flags", () => {
  assert.deepEqual(parseArgs(["@nessie/adapter-acpx-local", "--publish", "--skip-build"]), {
    help: false,
    selector: "@nessie/adapter-acpx-local",
    publish: true,
    skipBuild: true,
    otp: null,
  });
...
test("resolveTargetPackage matches by package name or dir", () => {
  const packages = [
    { dir: "packages/a", name: "@nessie/a", pkg: {} },
    { dir: "packages/b", name: "@nessie/b", pkg: {} },
  ];

  assert.equal(resolveTargetPackage("@nessie/a", packages).dir, "packages/a");
  assert.equal(resolveTargetPackage("./packages/b", packages).name, "@nessie/b");
});
