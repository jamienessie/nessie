...
import type { CreateIssueThreadInteraction, IssueDocumentSummary } from "@nessie/shared";
import { pluginOperationIssueOriginKind } from "@nessie/shared";
...
export function buildHostServices(
  ...
  options: { pluginWorkerManager?: PluginWorkerManager; manifest?: import("@nessie/shared").PaperclipPluginManifestV1 } = {},
  ...
  return {
    ...
    events: {
      ...
      async subscribe(params: { eventPattern: string; filter?: Record<string, unknown> | null }) {
        const handler = async (event: import("@nessie/plugin-sdk").PluginEvent) => {
          if (notifyWorker) {
